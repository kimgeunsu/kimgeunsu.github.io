...
- Policy Gradient
    ...
    - A3C
***
_목차를 보면서 흐름을 파악하고 되돌아왔을 때 내용을 기억 할 수 있으면 좋다._
***
...
강화학습이라고 불리는 방법은 기계학습에 속하는 다른 어떤 방법(지도학습, 비지도학습) 보다 상호작용으로부터 배우는 목표 지향적인 학습에 더욱 초점을 맞춘 방법이다.   
...
- S - states
- A - actions
- P - transition probability function (상태 천이 함수)
- R - reward function
- γ - discounting factor for future rewards (0-1)
...
__fig. state에서 state로 옮겨지는 확률이 표시된 Markov chain example__   
...
Bellman Equation 자체가 이전 Value Function과 다음 Value Function의 관계를 나타내기 때문에 이를 DP라고 할 수 있다. (뇌피셜)
...
Policy Evaluation과 Improvement를 반복해가며 Optimal value function 을 찾게 된다. (그림이 이해가지 않아도 된다.)   
...
3. DP는 다음 time step에 대해 모든 reward와 value function을 구한다.
   
   
1. MC: High variance, Zero bias, Not bootstap, Samples
2. TD: Low variance, Some bias, Bootstraps, Samples
3. DP: Bootstraps, Not sample
